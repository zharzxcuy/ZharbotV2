...
//============================CASE DASAR============================\\
				...
				break
                  ...
                  case 'creator':
                  case 'developer':
                  zharzx.sendMessage(from, {displayname: "ZHARZX", vcard: vcard1}, MessageType.contact, { quoted: mek})
	...
	case 'daftar':
				...
				if (isLimit(sender)) return reply(limitend(pushname))
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						zharzx.sendMessage(from, buffer, image, {quoted: mek, caption: '*_NIHKAK JANGAN LUPA SUBS CHANNEL [https://youtube.com/channel/UCpsA_nCZZ87CIjM6JHoi3sg]_*'})
						fs.unlinkSync(ran)
					})
...
//==============================SERTIFIKAT==================================//
		case 'tololserti': 				
				...
				zharzx.sendMessage(from, zhain, image, { quoted: mek, caption: '*_NIHKAK JANGAN LUPA SUBS CHANNEL [https://youtube.com/channel/UCpsA_nCZZ87CIjM6JHoi3sg]_*' })
				break
				case 'fuckboyserti': 				
				// ZX.BOT 
				if (!isRegistered) return reply(ind.noregis())
...
//==============================EDITTEXT==================================//
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    ...
                    zharzx.sendMessage(from, buffer, image, {caption: '*_NIHKAK JANGAN LUPA SUBS CHANNEL [https://youtube.com/channel/UCpsA_nCZZ87CIjM6JHoi3sg]_*', quoted: mek})
                    break
...
case 'deepfry':
	...
	case 'cium':
				...
				reply(ind.wait())
                  ...
                  zharzx.sendMessage(from, ini_buffer, image, { quoted: mek, caption: '*_NIHKAK JANGAN LUPA SUBS CHANNEL [https://youtube.com/channel/UCpsA_nCZZ87CIjM6JHoi3sg]_*'})
                    break
...
//ownermenu
				...
				if (!isAdmin) return reply('*Only Admin bot*')
					...
					reply(`Change Member limit To ${memberlimit} SUCCESS!`)
					break 
					case 'addbadword':  // Fix Bug By ZX.BOT* & ZHARZX*
                    if (!isOwner) return reply(ind.ownerb())
                    if (!isGroupAdmins) return reply(ind.admin())
...
//===========================MEDIA MENU===================================
	...
	case 'truth':
				...
				reply(ind.wait())
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                        const filePath = await zharzx.downloadAndSaveMediaMessage(encmedia, filename = getRandom());
				...
				reply(ind.wait())
                    ...
                    zharzx.sendMessage(from, get_buffer, image, { quoted: mek, caption: ini_txt })
                    break
...
zharzxgans = `*[ SAMEHADAKU ]*\n\n\nJudul : ${anu.title}\nDesc : ${anu.desc}\nLink : ${anu.link}`
zharzx.sendMessage(from, buffer, image, {quoted: mek, caption: zharzxgans})
break
case 'neonime':
if (!isRegistered) return reply(ind.noregis())
if (isLimit(sender)) return reply(ind.limitend(pusname))
...
teks += `*Title* : ${i.judul}\n*Link* : ${i.link}\n*Release* : ${i.rilis}\n=====================\n`
			    ...
			    zharzx.sendMessage(from, buffer, image, {quoted: mek, caption: zharz})
			    break
					case 'animegirl':
					if (!isRegistered) return reply(ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
